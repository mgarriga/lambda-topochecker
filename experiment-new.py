...
errcount        = 0
test_serverport = 'https://gl1m8pl7gh.execute-api.us-west-2.amazonaws.com/dev'
#test_serverport = 'http://localhost:5000'
...
class RequestTask(object):
    ...
    def __call__(self):
        ...
        resp2 = requests.get(test_serverport + '/check/R%28%5Btaxi%5D%2C%28%5BTRANSPORTSUBWAY%5D%20%7C%20%5BTRANSPORTBUSSTOP%5D%29%2C%5BFOODBAR%5D%29%20%26%20%28N%20%5BTRANSPORTFUEL%5D%29%20')
        #print(resp2.json())
        print(resp2.json()[u'load'])
        print(resp2.json()[u'decision'])
        checker_serverport = resp2.json()[u'serverport']
        print checker_serverport
       # resp = requests.get(config['checker_serverport'] +
        resp = requests.get(checker_serverport +
...
if __name__ == '__main__':
    ...
    NUM_PROCESSES = 100
    TIMEOUT = 30
    TIME_MULTIPLIER = 3 # should be an int
    MAX_LIMIT_TAXIPRESENCES = 50
    BEIJING_TIMELEN = 3600 * 1  # one hour
    BEIJING_STARTDATETIME = "2008-02-05 11:00:16"
